...
void loop() {
    ...
    static unsigned short DistanceD, DistanceG, DistanceF, lastD, lastG, durationG, durationD, durationF;
    ...
    if (voie3 < 1500) { // mode Manuel
        ...
        static float speedCoeff = 1.0;

        //if (millis() - t0 > 50) {
        //    t0 = millis();

        //if (DistanceG != 0) {
            AverageG = movingAverage(DistanceG, 1);
        //}
        //if (DistanceD != 0) {
            AverageD = movingAverage(DistanceD, 2);

            AverageF = movingAverage(DistanceF, 3);
        //}

            //Serial.println((String)AverageG + " | " + AverageD);
        //}

        /*if (lastD != DistanceD) {
          lastD = DistanceD;
          lcd.setCursor(0, 0);
          lcd.print((String)"DistanceD = " + AverageD + "      ");
        }*/

        /*if (lastG != DistanceG) {
          lastG = DistanceG;
          lcd.setCursor(0, 1);
          lcd.print((String)"DistanceG = " + AverageG + "      ");
        }*/

    /*if (AverageF < 20) {
        speedCoeff = 0.8;
    } if (AverageF < 15 && AverageF != 0) {
        if (AverageD >= AverageG) {
            //Serial.println((String) -35 + " IN 1ST " + 35);
            cmd_motor(1, 1, 35);
            cmd_motor(0, 0, 35);
            break;
        } else {
    ...
    } else if (AverageF == 0 || AverageF >= 15) {
        speedCoeff = 0.9;
    }

        /*if ((AverageD >= 40 || AverageD == 0) && (AverageG >= 40 || AverageG == 0 )) { // Droite et gauche clear
            cmd_motor(1, 1, int(50.0 * speedCoeff));
            cmd_motor(0, 1, int(50.0 * speedCoeff));
            //Serial.println((String) (50.0 * speedCoeff) + " IN MID " + (50.0 * speedCoeff));
        } else {
            unsigned short rspeed = AverageD;
            unsigned short lspeed = AverageG;
            if (rspeed == 0) {
                rspeed = int(50.0 * speedCoeff);
            }
            if (lspeed == 0) {
                lspeed = int(50.0 * speedCoeff);
            }
            //Serial.println((String)rspeed + " IN LAST " + lspeed);
            cmd_motor(1, 1, lspeed);
            cmd_motor(0, 1, rspeed);
        }

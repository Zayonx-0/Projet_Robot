...
void loop() {
    static unsigned long int t0, t1, inLoopTimer, inLoopResetTimer;
    static unsigned short DistanceDSon, DistanceGSon, DistanceFSon, lastD, lastG, durationG, durationD, durationF, DistanceD, DistanceG, DistanceF;
    ...
    if (voie3 < 1500) { // mode Manuel
        ...
        if (!(voie1 < 2080 && voie1 > 920) || !(voie2 < 2080 && voie2 > 920)) {
            cmd_motor(1, 0, 0);
            cmd_motor(0, 0, 0);
            return;
        }
        ...
        static unsigned short inLoop;

        if (millis() - inLoopResetTimer > 5000) {
            inLoopResetTimer = millis();
            lcd.print(inLoopResetTimer);
            inLoop = 0;
        }
        ...
        if (millis() - t0 > 25) {
            ...
            DistanceGSon = durationG * 0.034 / 2;
            DistanceDSon = durationD * 0.034 / 2;
            ...
            if (DistanceFSon == 0) DistanceFSon = 60;
            if (DistanceGSon == 0) DistanceGSon = 60;
            if (DistanceDSon == 0) DistanceDSon = 60;
            ...
            if (DistanceF < 20 || DistanceFSon < 20 || DistanceD < 20 || DistanceG < 20 || DistanceDSon < 20 || DistanceGSon < 20) {
                inLoopTimer = millis();
                inLoopResetTimer = inLoopTimer;
        
            while ((DistanceF < 25 || DistanceFSon < 25 || DistanceD < 25 || DistanceG < 25 || DistanceDSon < 25 || DistanceGSon < 25) && voie3 >= 1500) {
                if (millis() - t1 > 10) {
                    t1 = millis();
                    inLoop++;
                
                if (millis() - inLoopTimer > 2000 || inLoop > 150) {
                    inLoopTimer = millis();
                    inLoop = 0;
                    cmd_motor(1, 0, 45);
                    cmd_motor(0, 0, 55);
     ...
     else if (DistanceD > DistanceG || DistanceDSon > DistanceGSon) {
        speedD = floor(map(DistanceG, 20, 70, 30, 45) * 0.95);
        speedG = floor(map(DistanceD, 20, 70, 30, 45) * 1.05);
    } else if (DistanceD < DistanceG || DistanceDSon < DistanceGSon) {
        speedD = floor(map(DistanceG, 20, 70, 30, 45) * 1.05);
        speedG = floor(map(DistanceD, 20, 70, 30, 45) * 0.95);
    }

    cmd_motor(1, 1, speedD); // moteur droit
    cmd_motor(0, 1, speedG); // moteur gauche
    }

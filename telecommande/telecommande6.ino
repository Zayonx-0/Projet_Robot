...
void loop() {
    ...
    if (voie3 < 1500) { // mode Manuel
        ...
        static unsigned short resetInLoop;
        ...
        if (millis() - t0 > 30) {
            ...
            if (DistanceFSon < 20 || DistanceD < 20 || DistanceG < 20 || DistanceDSon < 20 || DistanceGSon < 20) {
                inLoop++;
            while (DistanceFSon < 25 || DistanceD < 25 || DistanceG < 25 || DistanceDSon < 25 || DistanceGSon < 25) {
                if (inLoop > 15) {
                    inLoop = 0;
                    cmd_motor(1, 0, 80);
                    cmd_motor(0, 0, 80);
        ...
        speedG = floor(speedG * 1.15);
        speedD = floor(speedD * 0.85);
    } else if (DistanceD < DistanceG) {
        speedD = floor(speedD * 1.15);
        speedG = floor(speedG * 0.85);
    }
